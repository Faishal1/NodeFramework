 ...
 * @static
 * @method getScorecardForOngoingAndCompleted
 * @param matches Matches document list
 * @return {undefined}
 */
Util.getScorecardForOngoingAndCompleted = async matches => {
  // Getting list of ongoing and completed matches
  const allMatches = [];
  const ocMatches = [];
  const ocMatchIds = [];

  matches.forEach(match => {
    const matchCopy = match.toJSON();
    allMatches.push(matchCopy);
    if (match.liveStatus !== "upcoming") {
      ocMatches.push(matchCopy);
      ocMatchIds.push(match.id);
    }
  });

  // Getting scorecard for ongoing and completed matches
  if (ocMatchIds.length) {
    const scorecards = await Scorecard.find({
      matchId: { $in: ocMatchIds },
      status: "active",
    });

    // Merging scorecards with match object
    scorecards.forEach(scorecard => {
      const scorecardJson = scorecard.toJSON();
      ocMatches.forEach(match => {
        const matchId = match._id.toString(); // eslint-disable-line
        if (
          scorecardJson.matchId.toString() === matchId &&
          scorecardJson.card
        ) {
          match.scorecard = scorecardJson.card; // eslint-disable-line
        }
      });
    });
  }
 ...
 * Getting scorecard for ongoing and completed matches
 * @static
 * @method getScorecardForMatch
 * @param matches Matches document list
 * @return {undefined}
 */
Util.getScorecardForMatch = async matches => {
  // Getting list of ongoing and completed matches
  const allMatches = [];
  const matchIds = [];
